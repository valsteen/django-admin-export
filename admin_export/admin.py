...
def export_simple_selected_objects(modeladmin, request, queryset):
    ...
    if len(selected) > 10000:
        ...
        return HttpResponseRedirect(reverse("export_to_xls") + "?ct=%s&ids=IN_SESSION" % (ct.pk,))
    else:
        return HttpResponseRedirect(reverse("export_to_xls") + "?ct=%s&ids=%s" % (ct.pk, ",".join(selected)))
